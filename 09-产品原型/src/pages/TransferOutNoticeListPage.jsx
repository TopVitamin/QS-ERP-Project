...
import { TransferOutNoticeActionDialogs } from '../components/erp/TransferOutNoticeActionDialogs.jsx';
...
import { transferOutNoticeColumns, transferOutNotices } from '../data/transferOutNoticeData.js';
...
/**
 * 调出通知单列表（F01）：无新增入口，页头仅导出；行内按状态互斥展示重试推送、取消。
...
const filterFields = [
  { key: 'noticeNo', label: '单号', type: 'search', placeholder: '请输入调出通知单号' },
  ...
  statusMultiSelectField('status', '单据状态', transferOutNoticeStatusLabels),
  { key: 'product', label: '商品', type: 'search', placeholder: '请输入商品编码' },
  { key: 'createdAtRange', label: '创建时间', type: 'date-range', placeholder: '不限' },
];

const initialVisibility = Object.fromEntries(transferOutNoticeColumns.map((column) => [column.key, true]));
const columnOptions = transferOutNoticeColumns.map((column) => ({ key: column.key, label: column.label }));
...
function handleCellClick(column, row, { onOpenPage }) {
  if (column.key === 'noticeNo') {
    onOpenPage?.('inventory-transfer-out-notice-detail', { row });
    return;
  }
...
function createRowActionHandler(openDialog) {
  return function handleRowAction(id, row, { notify }) {
    const latest = loadTransferOutNoticeById(row.id) || row;
...
export function TransferOutNoticeListPage(props) {
  const [dialog, setDialog] = useState(null);

  const listConfig = useMemo(() => ({
    title: '调出通知单',
    rows: transferOutNotices,
    storageKey: TRANSFER_OUT_NOTICE_STORAGE_KEY,
    initialFilters,
    initialPinnedKeys: ['noticeNo'],
    filterRows,
    initialVisibility,
    columns: transferOutNoticeColumns,
    columnOptions,
    filterFields,
    ...
    headerActions: [
      {
        ...
        render: (ctx) => (
          <ImportExportActions
            target={getTransferTarget('transfer-out-notice')}
            scopeSource={{ all: ctx.state.rows, filtered: ctx.state.filteredRows, selected: ctx.getSelectedRows() }}
            defaultColumnKeys={transferOutNoticeColumns.filter((column) => ctx.state.visibility[column.key] !== false).map((column) => column.key)}
            notify={ctx.notify}
    ...
    queryMessage: '已执行调出通知单查询',
    emptyText: '暂无调出通知单',
    emptyTextFiltered: '该条件下暂无调出通知单，可调整查询条件后重试',
