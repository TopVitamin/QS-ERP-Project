...
| 详情/页头状态 | 用 `StatusBadge` 胶囊；tone 与列表状态列语义一致 |
...
4. **弹窗**：列表页外包一层 `*ActionDialogs`，`onRowAction` 打开 dialog，不在列表里散写确认逻辑。弹窗标题、正文、Mock 规则见模块弹窗 PRD；文档结构见[《前端Demo_弹窗与Mock模板-V2》](../05-常用模板/02-模块设计交付/标准型页面&模块的PRD文档集/前端Demo_弹窗与Mock模板-V2.md)。收货通知单取消/重试/Mock 回传见 `PurchaseReceiptNoticeActionDialogs.jsx`。
