...
> 不写什么：不重复需求论证；不写采退发货通知单、采退出库单内部流程（见后续PRD）；不写财务ERP接口报文；不写系统权限与API设计。
...
| PUR-FR06：仓库、库存与财务ERP衔接 | F06、F05 | AC04 |
...
- 采退发货通知单、采退出库单内部流程与推送财务ERP细节（见对应PRD）；
- 采购退货变更单；推送财务ERP本单（财务ERP只接收采退出库单）；
...
- 采购退货单不向推送财务ERP；财务ERP只接收已审核采退出库单；
...
| 采退出库单审核回写 | 更新累计实出、可下推量 | 减少库存、消耗预占 | 财务ERP推采退出库单，不推本单 |
...
| 下游出库 | [《采退出库单主PRD》](../采退出库单/采退出库单主PRD.md) | 实出生成、回写与推送财务ERP见结果单PRD |
