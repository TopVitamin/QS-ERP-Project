...
> 不写什么：不重复需求论证；不写采退出库单内部流程与金蝶推送细节（见下游PRD）；不写仓库WMS作业细节与接口报文；不写系统权限与API设计。
...
通知单不增减库存，也不直接向金蝶推送；库存与业财影响由采退出库单承担。
...
| PUR-FR06：推送仓库、回写衔接、金蝶只接结果单 | F04、F07 | AC04、AC12 |
...
| R13 | 虚拟出库 | 发货处理方式=虚拟出库时，不调用仓库接口；创建成功后各行实出=通知数量、缺出=0，通知变为已发货，系统立即生成已审核采退出库单并推金蝶；占用被实际数量替代 | 不允许缺量或零出；保存后不可改处理方式；生成失败则整笔回滚，不留下通知；已发货后不能取消 |
...
| 下推创建（虚拟出库） | 产生已发货通知；实出=通知数量；占用被实际数量替代 | 无（由出库单记账） | 不推仓库；生成已审核采退出库单并推金蝶 |
...
| 采退出库 | [《采退出库单主PRD》](../采退出库单/采退出库单主PRD.md) | 生成、审核、金蝶推送不在本文 |
