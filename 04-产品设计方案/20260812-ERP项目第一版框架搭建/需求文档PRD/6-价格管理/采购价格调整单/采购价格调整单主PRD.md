...
| V0.3 | 明确采购税率由采购人员结合供应商纳税人类型手动填写，不从商品默认销售税率带出（2026-09-24用户确认） | 初稿，待评审 |
...
| R05 | 价税与精度 | 明细维护含税单价和税率，不含税单价由系统计算；含税单价必须大于零；税率允许0%；采购税率取决于供应商纳税人类型，由采购人员手动填写，不从商品默认销售税率带出，提交前必填；不含税单价＝含税单价÷(1＋税率÷100)，四舍五入保留4位小数；税率0%时等于含税单价 | 空值、零值、负值都不能提交；零值和负值在保存时即阻断（PRI-FR06、PRI-AC08）；精度依据PRI-FR04；税率来源见Q01（2026-09-24已确认） |
...
| 审核通过 | 审核状态→已审核（终态） | 按明细新增或覆盖采购价目表当前价 | 无；不推财务ERP、不产生库存 |
...
| --- | --- | --- | --- | --- | --- |
| Q01 | 采购调整明细的税率来源 | F02、R05；价目表税率 | 待指定 | 本PRD评审前 | 已定（2026-09-24，用户确认）：采购税率取决于供应商纳税人类型，不能从商品默认销售税率带出；采购人员手动填写，提交前必填 |
