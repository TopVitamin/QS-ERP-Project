...
通知单不直接增减库存，也不直接向金蝶推送；库存与业财影响由销退入库单承担。
...
| 下推创建（虚拟入库） | 产生已收货通知；实收=通知数量；占用被实际数量替代 | 无（由销退入库单记账） | 不推仓库；生成已审核销退入库单并推金蝶 |
| 推送成功 | 单据状态→待收货 | 无 | 仓库开始执行 |
| 收货回传（有实收） | 已收货；回写实收、缺收 | 无（由销退入库单记账） | 生成销退入库单，增加指定逻辑仓库存 |
| 零收回传 | 已取消 | 无 | 不生成销退入库单、不推金蝶 |
...
| 销退入库 | [《销退入库单主PRD》](../销退入库单/销退入库单主PRD.md) | 生成、审核、金蝶推送不在本文 |
