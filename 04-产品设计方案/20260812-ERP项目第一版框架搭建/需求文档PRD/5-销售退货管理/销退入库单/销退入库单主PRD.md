...
> 用途：定义销退入库单在ERP中的两条生成路径、自动审核、库存记账、来源回写及推送财务ERP，作为研发实现和测试验收的依据。
...
> 不写什么：不重复需求论证；不写仓库WMS作业细节与接口报文；不写财务ERP报文字段映射；不写外部渠道接入页面细节；不写系统权限与API设计。
...
| V0.4 | 人工确认入口调整：回到来源销退收货通知单详情页（2026-09-24确认；交互随对应模块下一轮修订补充） | 初稿，待评审 |
...
客户退回实物，仓库按销退收货通知单完成一次收货后，或渠道方在外部平台完成一笔退货入库后，需要形成实际入库结果单，作为库存增加和业财交接的唯一依据。销售退货单和销退收货通知单不直接增加库存，也不向推送财务ERP。
...
- 已审核结果逐单推送财务ERP；自动重试最多3次，仍失败则保持推送失败，在系统集成中心针对原单重推；销退入库单列表/详情不提供重推按钮；
...
- 接口拿不到仓库结果时的人工确认见 INT-FR06（沿来源通知，入口在来源销退收货通知单详情页，2026-09-24调整）；
...
| 编号 | 功能/位置 | 本次内容 | 需求来源 |
| --- | --- | --- | --- |
| F01 | 入库单列表 | 查询、列展示、进入详情；来源类型、审核状态与推送财务ERP状态多选筛选；勾选配合页头导出；**无新增**；一期批量栏无业务按钮 | SAL-FR05、SAL-FR09 |
| F04 | 详情页 | 分区域只读展示来源、明细价税、推送与审核信息 | SAL-FR05、SAL-FR10 |
| F08 | 重推财务ERP（系统集成中心） | 已审核且推送财务ERP失败时，在系统集成中心针对原单重试；本模块列表/详情不提供该按钮 | SAL-FR10、INT-FR03、INT-FR05 |
...
| SAL-FR10：仅两类结果单推财务ERP、沿用来源金额 | F04、F08 | AC04、AC13 |
...
| 核心职责 | 记录实际收回多少、入哪个仓、来源哪张通知或哪个外部订单；审核后记账并推送财务ERP |
...
| 场景ID | 场景 | 类型 | 说明 |
| --- | --- | --- | --- |
| S01 | 通知回传自动生成 | 主流程 | 已收货通知有实收→校验通过→生成入库单并自动审核→库存+→回写通知与退货单→推财务ERP |
| S02 | 通知少收 | 主流程 | 实收<通知量→入库按实收记账→退货单累计实退增加→缺收在通知侧展示、可下推量回补 |
| S03 | 通知零收 | 支线 | 通知按零收取消；**不生成销退入库单**；库存不变；不推财务ERP |
| S04 | 推送财务ERP失败重推 | 支线 | 已审核+推送失败→自动重试3次→仍失败则在系统集成中心重推原单；库存不回滚；本模块列表/详情无重推按钮 |
| S05 | 分批退货多次收货 | 主流程 | 同一退货单多张通知各自生成入库单，分别推送财务ERP |
| S06 | 虚拟入库自动生成 | 主流程 | 通知选虚拟入库并创建成功→按通知数量生成入库单并自动审核→库存+→回写→推财务ERP；不推仓库 |
| S07 | 外部ToC结果接收 | 主流程 | 渠道退货结果按来源拆分接收→匹配与库存校验通过→自动审核→记第三方逻辑仓→推财务ERP；不关联通知 |
| S08 | 外部ToC异常与完结保护 | 支线 | 匹配失败或库存不足→挂异常不记账不推财务ERP；重复回传不重复记账、完结结果不被改写（SAL-FR12、SAL-AC21） |
| S09 | 接口失败人工确认 | 支线 | 已交给仓库、接口拿不到结果→人工确认实际结果（INT-FR06）→沿来源通知形成入库单并提交审核 |

## 6. 状态与操作

状态定义 owner：[《单据与状态流转》](../../../系统架构和流程/03-单据与状态流转.md)第四节。销退入库单用**审核状态**与**推送财务ERP状态**两个字段并列维护。

**一期用户界面口径**：回传、虚拟入库或渠道结果生成的入库单创建即为**已审核**；列表与详情不提供新增，也不提供重推按钮。已交给仓库、接口拿不到结果时，人工确认实际结果走草稿 → 待审核，入口在来源销退收货通知单详情页（2026-09-24调整；人工确认交互随来源单据所属模块修订补充），不是本模块无来源建单。
...
| 草稿、待审核 | 人工确认实际结果时使用 | 否 | 入口不在本模块列表，在来源销退收货通知单详情页办理；见 R10（2026-09-24调整） |
...
两条路径生成的入库单创建时即为已审核，推送财务ERP状态从「未推送」开始。仓库回传、虚拟入库、外部来源等系统自动生成的结果单，创建人与最后更新人显示「系统」。
...
- 草稿、待审核状态本期只在人工确认实际结果时出现，入口不在本模块列表，在来源销退收货通知单详情页办理（2026-09-24调整）。
...
| --- | --- | --- | --- | --- | --- | --- |
| — | 自动生成（路径一·回传） | 来源通知已收货且有实收>0（R02）；校验通过（R03） | 已审核 | 无 | 分配单号（R01）；库存增加；回写通知/退货单（R05）；触发推送财务ERP（R06） | 校验失败保留异常，不记账 |
| — | 自动生成（路径一·虚拟入库） | 通知虚拟入库创建成功、实收=通知数量 | 已审核 | 无 | 同上；不推仓库 | 生成失败整笔回滚，通知不落已收货 |
| — | 自动生成（路径二·外部ToC） | 商品与逻辑仓匹配、库存校验通过（R04） | 已审核 | 无 | 同上；记第三方逻辑仓 | 匹配或库存不足→挂异常，不记账不推财务ERP（SAL-FR12） |
...
| — | 价格 | 路径一：含税单价、税率、不含税单价沿来源销售退货单交易价格；路径二：沿用来源结果金额，不重新定价（SAL-FR10） | 推送财务ERP时使用；推送范围已确认：全部价税字段逐单推送（2026-09-22） |
| — | 价税计算 | 价税合计=实际收货数量×含税单价；金额=实际收货数量×不含税单价；税额=价税合计−金额；尾差进税额 | 单价4位小数、金额与税额2位小数；税率允许0%（税额为0、价税合计等于金额） |
| — | 零收 | 通知回传实收为0时按零收取消该通知单，**不生成销退入库单** | 不改库存、不推财务ERP（SAL-FR06、业务设计 §4.4） |
...
| 规则编号 | 主题 | 规则 | 边界或例外 |
| --- | --- | --- | --- |
| R06 | 推送时机 | 已审核后由系统自动逐单推送财务ERP；销售侧仅销售出库单、销退入库单进入推送财务ERP | 退货单与通知单不推（SAL-FR10） |
...
| — | 业务日期 | 推送财务ERP时使用；**取实际收货时间的日期部分** | 与销售出库单同一口径 |
| — | 推送对应 | 逐单一一对应推送财务ERP，不按日期或店铺汇总 | 字段对照表由财务联调时给出 |
...
| 同一外部结果重复接收 | 已成功结果不重复记账、不重复形成财务ERP单据（SAL-AC21） |
...
| 外部ToC重复回传 | 已成功结果不得重复记账或形成财务ERP单据；完结结果不被来源改写覆盖 |
...
| --- | --- |
| 通知已有关联入库单 | 不可再自动生成 |
| 通知零收 | 按零收取消，不生成结果单、不改库存、不推财务ERP |
| 商品或逻辑仓匹配失败 | 挂异常，不审核、不记库存、不推财务ERP；补齐后对原记录重试（SAL-AC19） |
...
| --- | --- | --- | --- |
| AC01 | 自动路径/R03、R05 | 通知实收380并结束 | 自动生成入库单380件、已审核；单号 XTRK 规则；退货单累计实退+380；收货仓库存+380；触发推送财务ERP |
| AC02 | R02、零收 | 通知零收结束 | 不生成入库单；库存不变；不推财务ERP |
| AC03 | R05 | 同一退货单两批通知各实收50 | 两张入库单各50；退货单累计实退100；分别推送财务ERP |
| AC04 | F08/R08、R09 | 推送财务ERP明确失败 | 自动重试最多3次；仍失败则保持推送失败，可在系统集成中心重推；本模块列表/详情无重推按钮；库存不回滚 |
| AC05 | R07、F01 | 列表与详情 | 无新增/编辑入口；无行内重推；整单只读 |
| AC06 | F01 | 列表筛选来源类型、审核状态与推送财务ERP状态 | 多选 OR 匹配；列与筛选项与 TSV 一致 |
...
| AC11 | R04、SAL-AC21 | 对已成功且已推财务ERP的结果重复回传 | 不重复记账、不形成重复财务ERP单据；原完结结果不被覆盖 |
| AC12 | 业务日期、F01 | 查看列表默认排序与业务日期 | 业务日期=实际收货时间的日期部分；列表默认按业务日期倒序 |
| AC13 | SAL-FR10、SAL-AC17 | 接收含优惠后退货金额的外部ToC结果 | 金额沿用来源结果，不用ERP销售价目表重算；逐单一一对应推送财务ERP |
