...
- 记录B2B客户的成交商品、数量、价格、发货仓库和交期，作为后续分批发货的依据；
- 审核时校验可用库存，审核通过才占用库存；
...
| 核心职责 | 记录向谁销售、销售什么、从哪个仓发、销售多少及价格 |
...
| R02 | 客户与仓库 | 客户只能选择审核通过且启用的客户；发货仓库实际选择审核通过且启用的逻辑仓。一张订单一个仓库、一个交期，不同逻辑仓或交期必须拆单 | 变更客户后原匹配价格作废（PRI-FR05） |
...
| 规则编号 | 主题 | 规则 | 边界或例外 |
| --- | --- | --- | --- |
| R10 | 编辑范围 | 草稿可改；待审核不可改单只可走审核/撤回/取消；已审核不可改客户、仓库、商品、数量、价格和税率，只能调整交期 | 撤回回草稿后可再改；不做变更单；全部发足后不可调整交期（R12） |
| — | 审核记录 | 审核/撤回须记录审核人、审核时间；撤回意见保留 | 允许有权限用户提交并审核自己创建的单据（COM-FR01） |

审核后销售人员在订单「已审核+正常」且有可下推数量时，手工创建销售发货通知单；发货通知单创建时沿用订单发货仓库，客户可以按本次交付选择地址。现场自提仍须先有销售订单和销售发货通知单，仓库不得无单放货。
...
| --- | --- | --- | --- |
| AC01 | F01～F04/R01 | 创建B2B销售订单并提交审核 | 客户、仓库、交期、数量和价税信息完整时可提交；已审核后不能改量改价，仅可调交期 |
...
| 编号 | 问题 | 影响范围 | 状态/结论 |
| --- | --- | --- | --- |
| Q01 | 单据日期默认值及已审核后是否可改 | F02、列表 | **待确认**：上游销售规则未定义；本版字段清单不新增该字段 |
| Q02 | 信用提醒展示方式 | 列表、详情 | **待确认**：欠款、信用额度只提醒，不限制发货 |
| Q03 | 交付地址与物流 | 发货通知单 | **已确认**：按本次销售发货通知单维护；不在销售订单字段中重复 |
| Q04 | 草稿删除是否一期必做 | R15 | **已确认**：一期必做，无下游引用的草稿可删 |
| Q05 | 销售订单导入是否一期提供 | 列表页头 | **已确认**：正式导入不做；Demo 可保留导入向导作演示 |
