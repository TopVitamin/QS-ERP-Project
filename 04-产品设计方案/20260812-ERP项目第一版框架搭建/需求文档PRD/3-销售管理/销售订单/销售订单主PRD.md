...
> 不写什么：不重复需求论证；不写销售发货通知单、销售出库单内部流程；不写独立站订单或外部电商订单的履约；不写财务ERP接口报文、收款结算和系统权限实现。
...
| V0.5 | 确认详情页信用提醒；明确审核按商品、逻辑仓可用量校验并预占，同时记录库存流水 | 初稿，待评审 |
...
- 销售订单不直接完成收款结算；财务ERP不推送订单，只推出库结果；
...
| R07 | 审核占库 | 审核时按「商品＋发货逻辑仓」校验可用库存；同商品重复行按商品与逻辑仓汇总校验。任何一项不足时整单审核失败，不占库、不写流水。全部通过后按订单明细预占订单数量，并按逻辑仓＋商品记录一条「订单预占」库存流水 | 流水不改变即时库存；审核校验、预占、流水须整单成功或整单不生效。库存公式与流水字段见库存模块PRD |
| R08 | 可下推数量 | 销售数量−已结束通知的实际出库数量−未结束有效通知的通知数量；不同 SKU 不互抵；关单后不能再建通知 | 见 SAL-FR02 公式 |
| R09 | 实际出库回写 | 通知下发不重复占库，也不扣即时库存；实际出库后扣减即时库存和对应预占，写销售出库结果流水，并回写订单累计实际出库数量和发货状态 | 少出部分不回原通知补发，须另建通知；审核预占、实际出库与释放流水均可按销售订单追溯 |
...
| 规则编号 | 主题 | 规则 | 边界或例外 |
| --- | --- | --- | --- |
| R11 | 手动关闭 | 已审核+正常且仍有未发足余量时可手工关闭；关闭原因必填。关闭后不能新增通知，已有通知继续执行 | 关闭时释放未安排成通知的占库并写「订单释放」流水；执行中通知对应的占库保留，通知结束后再释放少出余量并写流水；不可反关单 |
...
| R14 | 取消 | 没有通知，或通知尚未推送、推送失败时可取消；存在推送中或已推送仓库且未取消的通知不允许直接取消；已向仓库申请取消须等回传；已有实际出库不能整单取消 | 取消时释放未执行预占并写「订单释放」流水；见 SAL-FR04 情形表 |
...
| 出库确认回写 | 更新累计实际出库数量、发货状态、可下推量；全部发货触发 R13 | 扣减即时库存和对应预占 | 财务ERP推出库结果，不推订单 |
...
| AC02 | F04/R07 | 按商品与逻辑仓校验销售订单审核，库存不足后补足再重试 | 可用库存按该商品、该逻辑仓判断；不足时审核失败且不占库、不写流水；足够时审核成功、预占订单数量并写「订单预占」库存流水 |
...
| Q02 | 信用提醒展示方式 | 详情、审核确认 | **已确认（2026-09-24）**：销售订单详情与审核确认提示关注客户欠款和信用情况；一期不读取或展示欠款/信用额度数值，仅提醒，不限制审核或发货 |
