...
> 用途：定义销售出库单在ERP中的生成、审核、库存记账、来源回写及推送财务ERP，作为研发实现和测试验收的依据。
...
> 不写什么：不重复需求论证；不写仓库WMS作业细节与接口报文；不写财务ERP报文字段映射；不写独立站订单页面细节；不写系统权限与API设计。

| 版本 | 本次变化 | 状态 |
| --- | --- | --- |
| V0.1 | 建立销售出库单主PRD，汇总B2B/独立站/外部ToC三类来源与页面分工 | 初稿，待评审 |
| V0.2 | 价税推送范围确认：全部价税字段逐单推送（Q03，用户确认） | 初稿，待评审 |
| V0.3 | 人工确认入口调整：回到来源销售发货通知单或独立站订单详情页（2026-09-24确认；交互随对应模块下一轮修订补充） | 初稿，待评审 |
...
仓库按销售发货通知单完成一次发货后，或独立站/外部渠道形成实际出库结果后，需要形成实际出库结果单，作为库存扣减和业财交接的唯一依据。销售订单和发货通知单不直接向推送财务ERP，也不单独完成库存扣减。
...
- 已审核结果逐单推送财务ERP；自动重试最多3次，仍失败则保持推送失败，在系统集成中心针对原单重推；销售出库单列表/详情不提供重推按钮；
...
| 编号 | 功能/位置 | 本次内容 | 需求来源 |
| --- | --- | --- | --- |
| F01 | 出库单列表 | 查询、列展示、进入详情；来源类型、审核状态与推送财务ERP状态多选筛选；勾选配合页头导出；**无新增**；一期批量栏无业务按钮 | SAL-FR10 |
| F04 | 详情页 | 分区域只读展示来源、明细价税、推送与审核信息 | SAL-FR02、SAL-FR10 |
| F08 | 重推财务ERP（系统集成中心） | 已审核且推送财务ERP失败时，在系统集成中心针对原单重试；本模块列表/详情不提供该按钮 | SAL-FR10、INT-FR03 |
...
| SAL-FR10：物流及财务ERP衔接 | F04、F08 | AC04、AC05 |
...
| 核心职责 | 记录实际出库多少、从哪个仓出、来源哪张通知/订单；审核后记账并推送财务ERP |
| 单据来源 | B2B须来自销售发货通知单；独立站来自独立站订单；外部ToC来自外部原始订单及结果 |
| 下游单据 | 无业务下游；财务ERP接收已审核结果；可作为销售退货溯源来源 |
...
| 场景ID | 场景 | 类型 | 说明 |
| --- | --- | --- | --- |
| S01 | B2B通知回传自动生成 | 主流程 | 已发货通知有实出→校验通过→生成出库单并自动审核→库存−→回写订单→推财务ERP |
| S02 | B2B通知少出 | 主流程 | 实出<通知量→出库按实出记账→订单累计实际出库增加→缺出在通知侧展示 |
| S03 | B2B通知零出 | 支线 | 不生成出库单；库存不变；不推财务ERP |
| S04 | 推送财务ERP失败重推 | 支线 | 已审核+推送失败→自动重试3次→仍失败则在系统集成中心重推原单；库存不回滚 |
| S05 | B2B分批出库 | 主流程 | 同一订单多张通知各自生成出库单，分别推送财务ERP |
| S06 | B2B虚拟出库自动生成 | 主流程 | 通知选虚拟出库并创建成功→按通知数量生成出库单并自动审核→库存−→回写订单→推财务ERP |
...
状态定义 owner：[《单据与状态流转》](../../../系统架构和流程/03-单据与状态流转.md)第四节。销售出库单用**审核状态**与**推送财务ERP状态**两个字段并列维护。

**一期用户界面口径**：回传或虚拟出库生成的出库单创建即为**已审核**；列表与详情不提供新增，也不提供重推按钮。已交给仓库、接口拿不到结果时，人工确认实际结果走草稿 → 待审核，入口在来源销售发货通知单或独立站订单详情页（2026-09-24调整；人工确认交互随来源单据所属模块修订补充），不是本模块无来源建单。
...
| 草稿、待审核 | 人工确认实际结果时使用 | 否 | 入口不在本模块列表，在来源销售发货通知单或独立站订单详情页办理；见 R10（2026-09-24调整） |
...
自动生成的出库单创建时即为已审核，推送财务ERP状态从「未推送」开始。仓库回传、虚拟出库、外部来源等系统自动生成的结果单，创建人与最后更新人显示「系统」。
...
| --- | --- | --- | --- | --- | --- | --- |
| — | 自动生成（B2B） | 来源通知已发货且有实出>0（R02）；校验通过（R03） | 已审核 | 无 | 分配单号（R01）；库存扣减；回写通知/订单（R05）；触发推送财务ERP（R06） | 校验失败保留异常，不记账 |
...
| — | 价格 | B2B/独立站：含税单价、税率、不含税单价、金额沿来源销售订单交易价格；ToC：沿用来源结果金额，不重新定价 | 推送财务ERP时使用；推送范围已确认：全部价税字段逐单推送（2026-09-22） |
...
| — | 业务日期 | 推送财务ERP时使用；**取实际出库时间的日期部分** | 见字段清单 Q02 |
...
| --- | --- | --- | --- |
| 自动生成并审核 | 已审核；带入实出数量与来源价格 | 逻辑仓库存扣减、消耗预占 | 触发推送财务ERP |
...
| 财务ERP重推 | 同一出库单重复推送不得重复扣减库存 |
| 外部ToC重复回传 | 已成功结果不得重复记账或形成财务ERP单据（SAL-FR12） |
...
| --- | --- | --- | --- |
| AC01 | 自动路径/R03、R05 | B2B通知实出380并结束 | 自动生成出库单380件、已审核；单号 XSCK 规则；订单累计实际出库+380；库存−380；触发推送财务ERP |
| AC02 | R02 | B2B通知零出结束 | 不生成出库单；库存不变；不推财务ERP |
...
| AC06 | F01 | 列表筛选来源类型、审核状态与推送财务ERP状态 | 多选 OR 匹配；列与 TSV 一致 |
...
| AC10 | SAL-FR02 | 同一订单两批通知各实出50 | 两张出库单各50；订单累计实际出库100；分别推送财务ERP |
| AC11 | R03、SAL-AC23 | B2B虚拟出库通知600件 | 不推仓库；生成已审核出库单600件；库存和预占各减600 |
| AC12 | SAL-FR09 | 接收外部ToC出库结果 | 校验通过后自动审核、记第三方逻辑仓、推财务ERP；金额沿用来源 |
...
| Q05 | 人工确认出库 | R10 | **已确认**：不提供无来源人工建出库单；已交给仓库、接口拿不到结果时，人工确认实际结果沿用 INT-FR06，入口在来源销售发货通知单或独立站订单详情页（2026-09-24调整；交互随来源单据所属模块修订补充） |
