...
- 在途差异例外：出库逻辑仓为虚拟在途仓时，不推送仓库，由系统按确认结果直接记账（§7.4）。
...
| S11 | 在途差异直接记账 | 例外 | 出库仓=虚拟在途仓→不推送仓库→系统按确认结果直接记账（§7.4） |
...
出库仓为虚拟在途仓时走直接记账路径：审核通过后不进入推送失败、待发货，由系统按确认结果直接生成已审核其他出库单，申请单直接完结为已发货（§7.4）。
...
| R03 | 业务类型 | 必填；枚举（2026-09-23确认）：盘亏、样品领用、赠送、报废、借出 | 盘亏为仓库主动回传场景，不通过本申请单办理 |
...
| 规则编号 | 主题 | 规则 | 边界或例外 |
| --- | --- | --- | --- |
| R20 | 虚拟在途仓例外 | 出库仓为虚拟在途仓时（用于处理分步式调拨少收差异）：审核通过后**不推送仓库**、不进入推送失败与待发货，由系统按申请数量直接生成已审核其他出库单并记账，申请单直接完结为已发货；数量按申请数量一次记账，不涉及缺量与取消中 | 在途差异保留与否由调拨规则决定，本单只负责按确认数量扣减在途逻辑仓（INV-FR03；《库存与仓储业务设计》§4.2）；在途差异的业务类型取「盘亏」（2026-09-23确认）；是否关联来源调拨单与路径标注待确认（Q07；在途差异路径本身已由INV-FR03确认，审核环节按现设计保留）；出库仓=在途仓的可用量校验按R07执行 |
...
| 业务类型 | 必填 | 同左 | 枚举（2026-09-23确认）：盘亏、样品领用、赠送、报废、借出 |
