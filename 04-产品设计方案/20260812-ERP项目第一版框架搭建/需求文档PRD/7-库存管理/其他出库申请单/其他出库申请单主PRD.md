...
> 不写什么：不写其他出库单内部的审核、库存记账与推送财务ERP（见[《其他出库单主PRD》](../其他出库单/其他出库单主PRD.md)）；不写其他入库申请单与结果单；不写库存查询、库存流水、分步式调拨的页面与流程规则；不写系统权限、接口报文与数据库设计。

| 版本 | 本次变化 | 状态 |
| --- | --- | --- |
| V0.1 | 建立其他出库申请单主PRD：建单、审核预占、自动推送、一次回传、缺量释放、取消、验收 | 初稿，待评审 |
| V0.2 | 同步09库存原型已交付状态；实现差异和Mock边界指向交付清单、Demo PRD与待改项 | 修订，待评审 |
| V0.3 | 确认虚拟在途逻辑仓可用于普通其他出库；在途仓盘亏走直接记账例外 | 修订，待评审 |
| V0.4 | 人工确认入口调整：回到本模块申请单详情页（2026-09-24确认；交互随本模块下一轮修订补充） | 修订，待评审 |
| V0.5 | 确认自动重试最多3次（2026-09-24） | 修订，待评审 |
| V0.6 | 确认在途仓盘亏按仓库存直接处理，不关联来源调拨单或另行标识路径 | 修订，待评审 |
...
非购销的出库（样品领用、赠送、报废等，清单已定，见§10.2 Q01）以及在途仓库存处理，都需要在ERP留下依据再交给仓库或系统执行。过去这类出库靠线下通知，既锁不住可用量，也说不清是谁批准、实际出了多少。
...
- 在途仓盘亏例外：虚拟在途仓可以像普通逻辑仓一样办理其他出库；选择「盘亏」时审核校验该仓可用量，不推送仓库，由系统按申请数量直接生成已审核其他出库单并记账（§7.4、R20），无需关联来源调拨单。其他出库类型按普通申请流程执行。
...
flowchart LR
    ...
    D -->|"回写"| B
    D -->|"已审核"| F["库存减少<br/>消耗本次预占<br/>推送财务ERP"]
    B -.->|"不推送"| F
    G["在途仓库存盘亏<br/>（虚拟逻辑仓）"] -.->|"不推仓库，系统直接记账"| D
```
...
| 其他出库申请单 : 来源单据 | 无强关联；在途仓盘亏按该仓库存处理，无需关联来源调拨单 |

### 3.4 本单据不负责的事项

- 申请单不直接减少库存：审核只预占，库存减少由已审核其他出库单记账；
- 申请单不向推送财务ERP；财务ERP只接收已审核其他出库单；
...
| 5 | 系统 | 实出大于0生成其他出库单并自动审核 | 即时库存减少、消耗实出对应预占、释放未出预占；申请单已发货；结果单推财务ERP |
...
| S07 | 推送失败重试 | 支线 | 已审核推送明确失败→推送失败→自动重试（最多3次，2026-09-24确认）→仍失败可人工重试或取消 |
...
| S11 | 在途仓盘亏直接记账 | 例外 | 出库仓=虚拟在途仓且业务类型=盘亏→审核校验可用量→不推送仓库→系统按申请数量直接生成已审核结果单并记账（§7.4、R20）；其他出库类型按普通申请流程执行 |
...
| 本单据不负责 | 其他出库单的审核与记账、推送财务ERP（见[《其他出库单主PRD》](../其他出库单/其他出库单主PRD.md)）；品质调整（直接调拨，INV-FR07）；仓库主动回传结果的补建（INV-FR04）；分步式调拨本身的执行与在途记账（INV-FR03） |
...
虚拟在途仓可用于普通其他出库。出库仓为虚拟在途仓且业务类型为「盘亏」时，审核前校验可用量；审核后不进入推送失败、待发货，由系统按申请数量直接生成已审核其他出库单，申请单直接完结为已发货（§7.4、R20）。该处理无需关联来源调拨单或另行标识路径；其他出库类型按普通流程推送仓库。
...
| 已审核 | 自动推送 | 系统作业 | 待发货（成功） | 无 | 推送成功；预占保持占用 | 明确失败→推送失败，自动重试（最多3次，2026-09-24确认） |
...
| 待发货 | 回传实出>0 | 实出按来源行回写且不超申请数量（R13、R14） | 已发货 | 无 | 生成其他出库单并自动审核；即时库存减少、消耗实出预占、释放未出预占；推财务ERP | 超量整次拒绝，保留异常 |
| 待发货 | 回传零出 | 最终确认整单零执行 | 已取消 | 无 | 实出显示0、未出＝申请数量；释放全部预占；不生成结果单、库存不变、不推财务ERP | 等待期间不提前按零出结束 |
| 取消中 | 回传实际结果 | 校验通过（R19） | 已发货 | 无 | 按真实结果完成；预占按实出消耗、未出释放 | 校验失败保留异常 |
| 已审核（在途仓盘亏） | 直接记账 | 出库仓为虚拟在途仓且业务类型为「盘亏」（R20） | 已发货 | 见Demo PRD | 校验可用量；不推送仓库；系统按申请数量生成已审核其他出库单并记账 | 记账校验不通过时阻断并报错；保持已审核，可按R17未推送取消或修复后重试，保留异常记录 |
...
| R02 | 出库仓 | 只能选择审核通过且启用的逻辑仓；一张申请单一个出库仓；一期不按库存状态限制用途，有足够可用量即可办理；虚拟在途仓也可选，按普通逻辑仓使用 | 已禁用逻辑仓不可新选，已有单据继续；在途仓盘亏例外路径见R20 |
| R03 | 业务类型 | 必填；枚举（2026-09-23确认）：盘亏、样品领用、赠送、报废、借出 | 建单可选范围（2026-09-24确认）：普通出库仓不建「盘亏」申请单，由仓库盘点回传形成结果单；虚拟在途仓可选「盘亏」用于处理该仓库存（见R20）；其余四项不受限 |
...
| R11 | 推送失败与重试 | 明确失败进入推送失败，系统自动重试最多3次（2026-09-24确认），仍失败可人工重试或按R17取消；失败、等待执行、取消中都不提前释放预占 | 重试间隔由技术配置；重试次数最多3次（Q06已定） |
...
| 规则编号 | 主题 | 规则 | 边界或例外 |
| --- | --- | --- | --- |
| R20 | 在途仓盘亏直接记账 | 出库仓为虚拟在途仓且业务类型为「盘亏」时，审核通过后先按R07校验可用量，再**不推送仓库**、不进入推送失败与待发货，由系统按申请数量直接生成已审核其他出库单并记账，申请单直接完结为已发货；无需关联来源调拨单或另行标识路径。虚拟在途仓的其他出库类型仍按普通流程审核、预占并推送仓库 | 本单按申请数量扣减在途逻辑仓库存（INV-FR03；《库存与仓储业务设计》§4.2）；时点与数量口径2026-09-24确认：审核通过即按申请数量直接记账；在途仓处理库存的业务类型取「盘亏」（2026-09-23确认） |
...
| 推送成功 | → 待发货 | 无 | 仓库接收执行要求 |
| 回传实出>0 | → 已发货，回写实出与未出 | 生成其他出库单，即时库存减少、消耗实出预占、释放未出预占 | 结果单推财务ERP，本单不推送 |
| 回传零出 | → 已取消 | 释放全部预占；库存不变 | 不推财务ERP |
| 取消（已审核） | → 已取消 | 释放未执行预占 | 不再推送 |
| 在途仓直接记账 | → 已发货 | 生成其他出库单，在途逻辑仓库存减少 | 结果单推财务ERP |
...
| 分步式调拨／在途 | 在途仓库存按盘亏处理，由业务人员另办其他出库扣减 | 出库仓=在途仓且业务类型为盘亏时走直接记账路径（R20） | AC14 |
| 财务ERP | 不接收申请单 | 只接收已审核其他出库单 | AC01 |
| 系统集成中心 | 推送失败记录查看（推送异常页签）；接口失败时人工确认实际结果在本单页面办理（INT-FR06，2026-09-24调整） | 失败原因与重推；人工确认入口在本模块申请单详情页 | AC11、Q09 |
...
| 结果单推送财务ERP失败 | 不回滚库存、预占消耗与申请单回写；在系统集成中心重推原结果单 |
...
| AC04 | R15、S03 | 仓库回传实出0 | 申请单按零出取消并释放全部预占，实出显示0、未出＝申请数量；不生成零数量结果单、库存不变、不推财务ERP（INV-AC06） |
...
| AC11 | F06/R11 | 推送明确失败 | 自动重试最多3次（2026-09-24确认）；仍失败保持推送失败，可人工重试或取消；等待期间预占不释放 |
...
| AC14 | F08/R20 | 出库仓选虚拟在途仓、业务类型选「盘亏」，在可用量范围内申请2件并审核 | 不推送仓库；系统按2件直接生成已审核其他出库单并扣减在途逻辑仓库存；申请单直接已发货 |
| AC15 | F09 | Demo Mock 模拟仓库回传 | 仅Demo可用；正式验收以仓库回传为准 |
| AC16 | R02、R07 | 出库仓选虚拟在途仓、业务类型选「报废」，提交并审核 | 允许选仓；按普通流程预占可用量、推送仓库并等待实出结果，不走直接记账路径 |
...
| Q02 | 出库仓允许选择虚拟在途仓的范围与路径标注（是否限在途差异） | 出库仓选项与例外路径适用范围 | — | — | 已确认（2026-09-24）：虚拟在途仓作为普通逻辑仓可用于其他出库；在途仓选择「盘亏」走R20直接记账，其他类型按普通流程执行；盘亏无需关联来源调拨单或另行标识 |
...
| Q07 | 在途仓盘亏是否关联来源调拨单或另行标识路径 | 单据关联与处理路径 | — | — | 已确认（2026-09-24）：不需要关联来源调拨单，也不需要另行标识；选择有可用库存的虚拟在途仓并用「盘亏」处理即可 |
...
| Q12 | 在途仓盘亏使用的业务类型 | 业务类型字段选项与在途仓处理路径 | — | — | 已定（2026-09-23）：并入现有值「盘亏」，不新增枚举项；无需来源调拨单关联 |
...
| 结果单 | [《其他出库单主PRD》](../其他出库单/其他出库单主PRD.md) | 生成、记账与推送财务ERP在结果单PRD |
