...
| V0.4 | 金蝶失败重推改到系统集成中心；拆开无来源建单与接口失败人工确认 | 初稿，待评审 |
...
- 已审核结果逐单推送金蝶；自动重试最多3次，仍失败则保持推送失败，在系统集成中心针对原单重推；采购入库单列表/详情不提供重推按钮；
...
| F08 | 重推金蝶（系统集成中心） | 已审核且金蝶推送失败时，在系统集成中心针对原单重试；本模块列表/详情不提供该按钮 | PUR-FR06、INT-FR03 |
...
| S06 | 虚拟入库自动生成 | 主流程 | 通知选虚拟入库并创建成功→按通知数量生成入库单并自动审核→库存+→回写订单→推金蝶 |
...
**一期用户界面口径**：回传或虚拟入库生成的入库单创建即为**已审核**；列表与详情不提供新增，也不提供重推按钮。已交给仓库、接口拿不到结果时，人工确认实际结果走草稿 → 待审核，入口形态见系统集成（待集成PRD），不是本模块无来源建单。
...
自动生成的入库单创建时即为已审核，金蝶推送状态从「未推送」开始。人工确认形成的入库单先走草稿 → 待审核，审核通过后再推送。仓库回传、虚拟入库等系统自动生成的结果单，创建人与最后更新人显示「系统」。
...
| — | 价格 | 含税单价、税率、不含税单价、金额沿来源采购订单交易价格 | 推送金蝶时使用；推送范围已确认：全部价税字段逐单推送（2026-09-22） |
...
| Q05 | 人工确认入库 | R10 | **已确认**：不提供无来源人工建入库单；已交给仓库、接口拿不到结果时，人工确认实际结果沿用《系统集成需求分析》INT-FR06，入口在系统集成中心；形成的入库单走提交审核 |
