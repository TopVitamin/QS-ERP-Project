...
> 用途：定义采购入库单在ERP中的生成、审核、库存记账、来源回写及推送财务ERP，作为研发实现和测试验收的依据。
...
| V0.3 | 增加虚拟入库自动生成路径；仍不提供人工建入库单 | 初稿，待评审 |
| V0.4 | 财务ERP失败重推改到系统集成中心；拆开无来源建单与接口失败人工确认 | 初稿，待评审 |
| V0.5 | 重试间隔本期不定秒数，由技术配置 | 初稿，待评审 |
| V0.6 | 列表按业务日期倒序；人工确认入口在系统集成中心 | 初稿，待评审 |
| V0.7 | 同步详细字段清单格式、页面骨架与Demo Mock口径 | 初稿，待评审 |
| V0.8 | 同步采购订单文档结构与表达方式 | 初稿，待评审 |
| V0.9 | 价税推送范围确认：全部价税字段逐单推送（Q03，用户确认） | 初稿，待评审 |
| V0.10 | 人工确认入口调整：回到来源采购收货通知单详情页（2026-09-24确认；交互随对应模块下一轮修订补充） | 初稿，待评审 |
...
关联资料：[《采购收货通知单主PRD》](../采购收货通知单/采购收货通知单主PRD.md)、[《采购订单主PRD》](../采购订单/采购订单主PRD.md)、[《01-系统流程与单据交互》](../../../系统架构和流程/01-系统流程与单据交互.md)第一节、[《02-实体对象关系》](../../../系统架构和流程/02-实体对象关系.md)、[《系统集成需求分析》](../../../../../02-需求调研和分析/07-系统集成/系统集成需求分析.md) INT-FR03、INT-FR06。

## 2. 需求背景与目标

### 2.1 背景

仓库按采购收货通知单完成一次收货后，需要形成实际入库结果单，作为库存增加和业财交接的唯一依据。采购订单和收货通知单不直接增加库存，也不向推送财务ERP。

没有统一的入库结果单时，实际收货数量、库存变化和财务输入难以与采购约定对齐，也无法逐单追溯分批收货过程。

### 2.2 目标

- 收货通知仓库收货并回传有实收结束后，或虚拟入库创建成功后，**系统自动**生成入库单并自动审核；
- 审核通过后增加对应逻辑仓库存，并回写通知实收、订单累计入库与收货状态；
- 已审核结果逐单推送财务ERP；自动重试最多3次，仍失败则保持推送失败，在系统集成中心针对原单重推；采购入库单列表/详情不提供重推按钮；
- 入库单整单只读，不提供新增、编辑、取消或作废；
- 已审核后不因推送财务ERP失败而回滚库存。
...
- 采购入库单列表、详情（只读查看）；
- 由收货通知回传或虚拟入库触发的自动生成（本模块无新增入口）；接口失败时人工确认见 INT-FR06；
...
| 编号 | 功能/位置 | 本次内容 | 需求来源 |
| --- | --- | --- | --- |
| F01 | 入库单列表 | 查询、列展示、进入详情；审核状态与推送财务ERP状态多选筛选；勾选配合页头导出；**无新增**；一期批量栏无业务按钮 | PUR-FR06 |
| F04 | 详情页 | 分区域只读展示来源、明细价税、推送与审核信息 | PUR-FR02、PUR-FR06 |
| F08 | 重推财务ERP（系统集成中心） | 已审核且推送财务ERP失败时，在系统集成中心针对原单重试；本模块列表/详情不提供该按钮 | PUR-FR06、INT-FR03 |
...
| PUR-FR06：库存记账、财务ERP只接结果单 | F04、F08 | AC03、AC04 |
...
- **无来源人工建入库单**、列表/详情新增入口（回传或虚拟入库自动生成的已审核单不走草稿编辑）；
- 入库单导入；
- 已审核后修改数量、价格、仓库或反审核；
- 零数量入库单；
...
| 唯一来源 | 必须关联采购收货通知单。常规：仓库回传有实收并结束，或虚拟入库创建成功后，系统自动生成。另：已交给仓库、接口拿不到结果时，可人工确认实际结果（INT-FR06），仍须沿来源通知，不提供无来源建单 |
| 与原型差异 | 一期不提供无来源人工建入库单；列表/详情无重推按钮。以 `09-产品原型` 采购入库单页面为准，原型尚未实现虚拟入库，也尚未把重推改到系统集成中心 |
...
| 核心职责 | 记录实际入库多少、入哪个仓、来源哪张通知与订单；审核后记账并推送财务ERP |
| 单据来源 | 必须来自采购收货通知单。常规由回传或虚拟入库自动生成；接口拿不到仓库结果时，可人工确认实际结果，仍须沿来源通知，不提供无来源建单 |
...
| 场景ID | 场景 | 类型 | 说明 |
| --- | --- | --- | --- |
| S01 | 通知回传自动生成 | 主流程 | 已收货通知有实收→校验通过→生成入库单并自动审核→库存+→回写订单→推财务ERP |
| S02 | 通知少收 | 主流程 | 实收<通知量→入库按实收记账→订单累计入库增加→缺收在通知侧展示 |
| S03 | 通知零收 | 支线 | 不生成入库单；库存不变；不推财务ERP |
| S04 | 推送财务ERP失败重推 | 支线 | 已审核+推送失败→自动重试3次→仍失败则在系统集成中心重推原单；库存不回滚；本模块列表/详情无重推按钮 |
| S05 | 分批收货 | 主流程 | 同一订单多张通知各自生成入库单，分别推送财务ERP |
| S06 | 虚拟入库自动生成 | 主流程 | 通知选虚拟入库并创建成功→按通知数量生成入库单并自动审核→库存+→回写订单→推财务ERP |
| S07 | 接口失败人工确认 | 支线 | 已交给仓库、接口拿不到结果→人工确认实际结果（INT-FR06）→沿来源通知形成入库单并提交审核；不是无来源建单 |

## 6. 状态与操作

状态定义 owner：[《单据与状态流转》](../../../系统架构和流程/03-单据与状态流转.md)第四节。采购入库单用**审核状态**与**推送财务ERP状态**两个字段并列维护。

**一期用户界面口径**：回传或虚拟入库生成的入库单创建即为**已审核**；列表与详情不提供新增，也不提供重推按钮。已交给仓库、接口拿不到结果时，人工确认实际结果走草稿 → 待审核，入口在来源采购收货通知单详情页（2026-09-24调整；人工确认交互随来源单据所属模块修订补充），不是本模块无来源建单。
...
| 草稿、待审核 | 人工确认实际结果时使用 | 否 | 入口不在本模块列表，在来源采购收货通知单详情页办理；见 R10（2026-09-24调整） |
...
自动生成的入库单创建时即为已审核，推送财务ERP状态从「未推送」开始。人工确认形成的入库单先走草稿 → 待审核，审核通过后再推送。仓库回传、虚拟入库等系统自动生成的结果单，创建人与最后更新人显示「系统」。
...
| --- | --- | --- | --- | --- | --- | --- |
| — | 自动生成 | 来源通知已收货且有实收>0（R02）；校验通过（R03） | 已审核 | 无 | 分配单号（R01）；库存增加；回写通知/订单（R05）；触发推送财务ERP（R06） | 校验失败保留异常，不记账 |
| 已审核 | 自动推送 | 系统作业 | 推送中 | 无 | — | — |
| 推送中 | 推送成功 | 财务ERP确认 | 推送成功 | 无 | 记录推送时间 | — |
| 推送中 | 推送失败 | 财务ERP明确失败 | 推送失败 | 无 | 记录推送失败原因；**不回滚库存**；触发自动重试（R09） | — |
...
| R03 | 自动生成 | 仓库回传有实收并结束，或虚拟入库创建成功后，校验通过则系统自动生成并自动审核 | 不经人工建单或人工审核 |
| R10 | 人工确认 | 已交给仓库、接口拿不到结果时，可按实际发生情况确认并沿来源通知形成入库单，走提交审核（INT-FR06） | 不能无来源新建入库单；不能用来替代虚拟入库 |
...
| — | 价格 | 含税单价、税率、不含税单价、金额沿来源采购订单交易价格 | 推送财务ERP时使用；推送范围已确认：全部价税字段逐单推送（2026-09-22） |
...
| 规则编号 | 主题 | 规则 | 边界或例外 |
| --- | --- | --- | --- |
| R06 | 推送时机 | 已审核后由系统自动逐单推送财务ERP | 订单与通知不推送 |
| R09 | 自动重试 | 推送明确失败后系统自动重试，**最多3次**；3次仍失败则保持「推送失败」，等待在系统集成中心重推（F08） | 间隔本期不定秒数，由技术配置 |
| R08 | 失败重推 | 在系统集成中心针对原单重试，不另建业务单；采购入库单列表/详情不提供重推按钮 | 不回滚库存与回写 |
| — | 业务日期 | 推送财务ERP时使用；**取实际入库时间的日期部分** | 见字段清单 |
...
| --- | --- | --- | --- |
| 自动生成并审核 | 已审核；带入实收数量与来源价格 | 逻辑仓库存增加 | 触发推送财务ERP |
...
| F09 | Demo Mock | 非正式验收 | [弹窗与Mock](前端Demo版PRD/采购入库单前端Demo版PRD_弹窗与Mock.md) §2 |
...
| --- | --- | --- | --- |
| AC01 | 自动路径/R03、R05 | 通知实收380并结束 | 自动生成入库单380件、已审核；单号 CGRK 规则；订单累计入库+380；库存+380；触发推送财务ERP |
| AC02 | R02、PUR-AC10 | 通知零收结束 | 不生成入库单；库存不变；不推财务ERP |
| AC03 | R05、PUR-AC07 | 同一订单两批通知各实收50 | 两张入库单各50；订单累计入库100；分别推送财务ERP |
| AC04 | F08/R08、R09 | 推送财务ERP明确失败 | 自动重试最多3次；仍失败则保持推送失败，可在系统集成中心重推；本模块列表/详情无重推按钮；库存不回滚 |
| AC05 | R07、F01 | 列表与详情 | 无新增/编辑入口；无行内重推；整单只读 |
| AC06 | F01 | 列表筛选审核状态与推送财务ERP状态 | 多选 OR 匹配；列与 TSV 一致 |
| AC07 | F04 | 详情展示来源通知与订单 | 可跳转来源单据；展示推送记录 |
| AC08 | F09 | Demo Mock 模拟自动生成 | 仅 Demo 可用；正式验收以回传为准 |
| AC09 | R10 | 已交给仓库的通知接口拿不到结果 | 可人工确认实际结果，沿来源通知形成入库单并提交审核；不能无来源建单 |
...
| Q05 | 人工确认入库 | R10 | **已确认**：不提供无来源人工建入库单；已交给仓库、接口拿不到结果时，人工确认实际结果沿用《系统集成需求分析》INT-FR06，入口在来源采购收货通知单详情页（2026-09-24调整；交互随来源单据所属模块修订补充）；形成的入库单走提交审核 |
