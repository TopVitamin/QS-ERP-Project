...
| R08 | 编辑范围 | 草稿可改；待审核不可改单只可走审核/撤回/取消；已审核不可改数量价格，只能调整交期 | 单据日期默认当天，草稿可改，提交后不可改；撤回回草稿后可再改；不做变更单；全部收足后不可调整交期（R12） |
...
| 编号 | 问题 | 影响范围 | 状态/结论 |
| --- | --- | --- | --- |
| Q01 | 单据日期默认值及已审核后是否可改 | F02、列表 | **已确认**：默认当天；草稿可改；提交后不可改 |
...
| Q06 | 采购订单导入是否一期提供 | 列表页头 | **已确认**：正式导入不做；Demo 可保留导入向导作演示 |
