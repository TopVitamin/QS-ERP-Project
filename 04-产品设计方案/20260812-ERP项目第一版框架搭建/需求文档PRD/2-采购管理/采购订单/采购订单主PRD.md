...
- 到期自动关单参数配置界面（宽限规则待确认，系统作业另定）；
- 采购订单导入导出、系统权限矩阵、API/表结构/并发锁实现；
- 直推采购入库单、缺量完结、审核通过自动生成通知单（与当前 02/03/04 口径冲突，一期不做）。

| 范围补充 | 说明 |
| --- | --- |
| 适用对象 | 成品采购；一张订单一个收货仓库、一个承诺交期；不同仓库或交期拆单 |
...
| 采购订单 : 收货仓库 | N:1；一张订单一个收货仓库 |
| 采购订单 : 采购订单明细 | 1:N |
| 采购订单 : 采购收货通知单 | 1:N；分批创建 |
| 采购订单明细 : 通知单明细 | 1:N；按 SKU 分行占用与回补 |
| 采购收货通知单 : 采购入库单 | 1:1 或 1:N（按回传批次，见通知/入库 PRD） |
...
| S07 | 草稿删除 | 支线 | 草稿+正常且未被下游引用时可物理删除（Q05 待确认是否一期必做） |
...
| 功能编号 / 动作 | 草稿+正常 | 待审核+正常 | 已审核+正常 | 已审核+已关闭 | 已审核+已取消 | 草稿/待审核+已取消 |
| --- | --- | --- | --- | --- | --- | --- |
| F02 编辑 | ✅ | ❌ | ❌ | ❌ | ❌ | ❌ |
| F03 提交 | ✅ | ❌ | ❌ | ❌ | ❌ | ❌ |
| F03 取消 | ❌ | ✅ | ❌ | ❌ | ❌ | ❌ |
...
列表与详情行内操作按状态互斥展示：草稿仅提供删除（不提供取消、关闭）；待审核不提供删除与关闭；已审核+正常时取消与关闭可同时出现（取消须满足R09，关闭与调整交期须满足R12）。**已审核+正常+全部收货**（S08）：仅查看，不提供下推、取消、关闭、调整交期。列表批量操作栏一期仅展示已选中计数与列设置，不提供批量删除、提交、审核、取消、关闭；导入导出在页头，导出可选已勾选范围。
...
| 已审核+正常 | 关闭 | R10、R12；仍有未收足余量 | 已审核+已关闭 | 必填关闭原因 | 不可新增通知；已有通知继续 | 全部收足时阻断；不可撤销 |
...
| R05 | 价税与金额 | 行价税合计=采购数量×含税单价；行金额=采购数量×不含税单价；行税额=价税合计−金额；单头价税合计、税额、金额分别汇总明细对应字段 | 精度与尾差待确认（Q03） |
...
| R08 | 编辑范围 | 草稿可改；待审核不可改单只可走审核/撤回/取消；已审核不可改数量价格，只能调整交期 | 不做变更单；撤回回到草稿后可编辑；全部收足后不可调整交期（R12） |
...
| R10 | 关闭 | 已审核+正常且仍有未收足余量时可手工关闭；到期自动关闭宽限参数待确认；关闭不可撤销，不能反关单 | 已有通知继续执行，不新增通知；全部收足后不可关闭（R12） |
| R12 | 履约完结 | 收货状态=全部收货时，视为采购履约已执行完毕；订单保持已审核+正常+全部收货，不自动关单 | 不提供关闭、调整交期、下推、取消；仅查看追溯 |
| R11 | 删除 | 仅草稿+正常且未被下游引用时可删除 | 待确认是否一期必做（Q05） |
...
| F03 | 提交/待审取消 | R04、R08；§6.5 | [弹窗与Mock](前端Demo版PRD/采购订单前端Demo版PRD_弹窗与Mock.md) §一 |
| F04 | 审核/撤回 | R08；§6.5 | 弹窗 PRD §二（审核、撤回） |
| F05 | 详情 | R06、R08～R10；§6.4 | [详情页](前端Demo版PRD/采购订单前端Demo版PRD_详情页.md) |
| F06 | 交期/关单/取消/下推 | R07～R10；§6.4 | 列表/详情行操作 + 弹窗 PRD §二 |
| F07 | Demo Mock | 非正式验收 | 弹窗 PRD §三 |
...
| 编号 | 问题 | 影响范围 | 状态/结论 |
| --- | --- | --- | --- |
| Q01 | 单据日期默认值及已审核后是否可改 | F02、列表 | 待确认 |
| Q02 | 到期自动关单宽限参数 | R10、定时任务 | 待确认（PUR-FR03） |
| Q03 | 价税合计、税额、金额精度与尾差 | R05 | 待确认 |
| Q04 | 列表默认排序字段 | F01 | Demo 暂定最后更新时间倒序 |
| Q05 | 草稿删除是否一期必做 | R11 | 待确认 |
