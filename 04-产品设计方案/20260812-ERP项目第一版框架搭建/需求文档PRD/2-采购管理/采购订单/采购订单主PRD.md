...
> 不写什么：不重复需求论证；不写采购收货通知单、采购入库单内部流程（见后续PRD）；不写财务ERP接口报文；不写预付款系统流程；不写系统权限与 API 设计。
...
- 采购订单不直接完成应付结算与付款；财务ERP不推送订单，只推入库结果；
...
| 创建收货通知 | 占用可下推量 | 无 | 无 |
| 入库确认回写 | 更新累计入库数量、剩余入库数量、收货状态、可下推量 | 增加库存 | 财务ERP推入库结果，不推订单 |
| 关闭 | 业务状态→已关闭 | 不新增库存 | 无 |
